...
document.addEventListener('DOMContentLoaded', () => {
    if (!isLocalFile && typeof firebase !== 'undefined' && firebaseConfig.apiKey !== "YOUR_API_KEY") {
        ...
        const groceryInput = document.getElementById('grocery-item-input'); // 撠? HTML #grocery-item-input
        const btnAddGrocery = document.getElementById('add-grocery-btn');  // 撠? HTML #add-grocery-btn
        ...
        if (groceryListEl && groceryInput && btnAddGrocery) {
            ...
            groceryListRef.on('value', (snapshot) => {
                ...
                Object.keys(data).forEach(key => {
                    ...
                    itemEl.innerHTML = `
                        <span class="grocery-name">${item.name}${item.assignee ? '<span class="badge-assignee">' + item.assignee + '</span>' : ''}</span>
